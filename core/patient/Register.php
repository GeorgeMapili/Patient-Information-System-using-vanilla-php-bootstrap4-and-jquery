...
var_dump(ini_set('display_startup_errors', '1'));
var_dump(error_reporting(E_ALL));exit;
...
class register extends Database
{
    ...
    {
        if (!preg_match("/^([a-zA-Z' ]+)$/", $this->name)) { //!preg_match("/^[^\s]+( [^\s]+)+$/",$this->name) //!preg_match("/^([a-zA-Z' ]+)$/", $this->name)
